...
DecisionRule throw {
  ...
  { (notheld OBJ) (grasped X OBJ)! (busy X)! (dynamic OBJ) (dynamicOn ANY OBJ)! (stable ANY OBJ)! (stableOn ANY OBJ)! } # komoThrow(OBJ)=1. }
}
...
DecisionRule hit {
  ...
  { (animate OBJ) (on OBJ ANY)! komo(touch BAT OBJ) komo(impulse BAT OBJ) (dynamic OBJ)  } # komoHit(BAT, OBJ) }
}
